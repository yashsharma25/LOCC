...
def main():
    psi = Statevector(np.sqrt(1/2) * np.kron(ket0, ket0) + np.sqrt(2/5)  * np.kron(ket1, ket1)  + np.sqrt(1/10) * np.kron(ket2, ket2))
    phi = Statevector(np.sqrt(3/5) * np.kron(ket0, ket0) + np.sqrt(1/5)  * np.kron(ket1, ket1)  + np.sqrt(1/5) * np.kron(ket2, ket2))

    psi = k_party(2, 3, [(1, [3]), (1, [3]), (1, [2])], psi)
    phi = k_party(2, 3, [(1, [3]), (1, [3]), (1, [2])], phi)
    psi.is_transformable(phi)
